...
| Focused Tests | 初轮为`18 passed`；独立复审修复后扩充为`25 passed`，覆盖条件环、成员输出归档、本机文件验证、非法类型结构化失败和阶段三台账隔离。 |
...
| Full Regression | 首次误用数学建模Python导致缺少NeuroKit2，未进入测试；改用冻结Python 3.14.4并完成修订后为`496 passed in 17.44s`。 |
