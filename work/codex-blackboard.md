...
记录用户确认统一120秒模块时间线：15秒场景原生示范、90秒真实设备闭环、15秒锁定累计状态并自动转场，并明确主要过程分析窗口和跨制品阶段日志。
...
- 用户已确认每个模块固定 `120 s`：`demo` 0-15秒、`closed_loop` 15-105秒、`lock_transition` 105-120秒；四模块主体验共480秒。
- 只有中央90秒 `closed_loop` 进入主要过程估计；前后阶段继续记录原始数据，但仅用于质量、适应过程和稳健性分析。
- 末15秒锁定105秒时的累计恢复状态，不得强制跳到完全恢复；该阶段自动完成一致转场，不增加参与者操作。
- 四模块阶段边界完全一致；不同呼吸流程实际完成的周期数作为流程差异记录，不通过改变阶段时长抹平。
...
- 下一道门是定义承担唯一确认性顺序问题的低自由度理论对比；14自由度结构模型保持次要地位。
...
- 只有90秒主要过程窗口，某些流程可用周期数有限；预试必须验证相位与周期特征在该窗口内具有足够可辨识性。

## Next-Step Queue

1. 已追加 U39，冻结统一120秒三段时间线、主要过程窗口、锁定语义和最小阶段日志。
2. 验证三段边界、总时长、分析资格、转场语义、断流标记、量表时点、术语和 diff 格式。
3. 仅暂存本任务文件，提交并推送。
4. 下一轮继续 Grill：定义唯一确认性低自由度顺序对比。
