...
记录用户对结构化顺序分析层级的确认：位置和有向相邻转换承担主要分析，24 种完整顺序承担次要估计，策略学习后使用新参与者独立确认。
...
- 下一道门是冻结位置与转换的主要联合假设、编码方式和个别系数检验层级；随后才能进行功效分析和样本量设计。
...
1. 已追加 U31，冻结位置、转换、完整顺序和策略确认的证据层级。
2. 验证分析层级、24 顺序保留方式、术语和 diff 格式。
3. 仅暂存本任务文件，提交并推送。
4. 下一轮继续 Grill：冻结位置与转换的主要联合假设和检验层级。
