...
记录用户对单一全局主要检验、条件性位置与转换效应组检验、Holm 校正和探索结果闸门的确认。
...
- 下一道门是冻结功效设计标准：目标功效、最小关注效应、参数来源和模拟方案；随后计算第一阶段样本量。
...
1. 已追加 U32，冻结全局主要检验、条件性效应组检验和 Holm 闸门。
2. 验证模型层级、校正规则、探索边界、术语和 diff 格式。
3. 仅暂存本任务文件，提交并推送。
4. 下一轮继续 Grill：冻结功效设计标准和参数来源。
