...
记录用户确认四天气—呼吸复合模块映射：将四组对应关系冻结为研究设计基线，同时保留正式运行参数的预试门禁和版本化修订要求。
...
- 下一道门是确认参与者侧呼吸提示由场景原生机制还是统一呼吸圈承担主要职责，之后冻结120秒内部时间线并定义低自由度理论顺序对比。
...
1. 已追加 U37，将四模块映射提升为已确认研究设计基线，并保留正式运行参数的预试门禁。
2. 验证四组映射、旧方案覆盖、无手部操作、固定配对解释边界、参数验证边界、术语和 diff 格式。
3. 仅暂存本任务文件，提交并推送。
4. 下一轮继续 Grill：单独确认场景原生机制与统一呼吸圈的提示职责。
