...
记录用户确认重构论文核心贡献层级：以场景原生呼吸引导框架和可解释闭环序列编排方法作为贡献对象，将24顺序因果评估、真实设备记录、策略学习和独立比较降为证据链。
...
1. 已追加 U40，冻结两项核心贡献、证据链角色、识别边界和覆盖关系。
2. 验证核心贡献名称、映射语法、编排方法最低要求、三阶段证据链、固定配对边界、基准条件和下一道门。
3. 仅暂存本任务文件，提交并推送。
4. 下一轮继续 Grill：冻结场景原生呼吸引导框架的唯一主要经验主张及其最小比较基准。
