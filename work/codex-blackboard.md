...
在补读天气场景变化设计后，联网复核四种权威呼吸流程，形成与既有风暴、炙烤、暴雪、褪色恢复机制一致的候选映射，并保持为待用户确认状态。
...
- 当前候选映射为：风暴—`3-3-3-3` 箱式、炙烤—`4-6`、暴雪—`5-5`、褪色—循环叹息。映射及精确参数均待用户确认。
...
- 下一道门是重新冻结四个天气—呼吸复合模块的标准化节律、作用维度和被动呈现方式，之后定义低自由度理论顺序对比。
...
1. 已追加 U36，记录联网证据、天气变化设计依据、证据边界和四模块候选映射。
2. 验证来源链接、场景机制、候选参数、待确认状态、术语和 diff 格式。
3. 仅暂存本任务文件，提交并推送。
4. 下一轮继续 Grill：先确认四模块候选映射，再单独确认场景原生提示和120秒内部时间线。
