...
- 抽象节律提示基准、`protocol_fidelity`、非劣界值和完整项目第三比较条件仍是建议方案，不因写入汇总而升级为确认决定。
...
- 原始Grill记录U40确认两项长期贡献，U41明确抽象对照、`protocol_fidelity`、非劣设计和样本量尚未确认。
...
- 新确认门：Gate 1唯一主要设备结果是否采用中央90秒有效周期比例 `protocol_fidelity`；确认前不拆四人任务包。
