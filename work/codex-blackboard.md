...
记录用户对 `90%` 目标功效、基于24种均衡顺序满秩矩阵的蒙特卡洛模拟、预实验参数用途和24倍数取整规则的确认。
...
- 下一道门是冻结最小关注效应的表达尺度、敏感性网格和不可用会话预留比例；随后运行正式样本量模拟。
...
1. 已追加 U33，冻结功效模拟标准，并记录24顺序设计矩阵秩检查结果。
2. 验证目标功效、参数来源、取整规则、自由度、术语和 diff 格式。
3. 仅暂存本任务文件，提交并推送。
4. 下一轮继续 Grill：冻结最小关注效应网格和不可用会话预留比例。
