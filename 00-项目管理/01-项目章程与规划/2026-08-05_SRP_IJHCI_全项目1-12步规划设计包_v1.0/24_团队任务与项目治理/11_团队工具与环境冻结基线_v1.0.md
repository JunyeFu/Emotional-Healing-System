...
- 维护设计依据、来源等级和引用导出，不使用个人孤立文献库替代共享库。
...
- 维护许可、来源、招募与论文证据索引，并运行Python校验、fixture和日志检查。
...
3. 正式领取时把`common`替换为`design`、`unity`、`python_data`或`experiment_td_governance`，用于在证据中标记领取岗位；五个模式检查同一套全员工具和版本。
...
因此当前中枢机不能标记为“全员公共基线完整”。Unity、TouchDesigner和Python数据处理栈已经满足全员工具要求；唯一公共缺口是Zotero `9.0.6`。安装前不影响F-01、F-03或F-04工程启动，但会阻断共享文献库验收和后续论文引用协作。
