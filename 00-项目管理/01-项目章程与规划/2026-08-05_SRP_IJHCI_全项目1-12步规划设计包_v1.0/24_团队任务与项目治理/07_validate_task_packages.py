...
def main() -> int:
    ...
    if len(rows) != 57:
        errors.append(f"expected 57 registry entries, found {len(rows)}")
    ...
    if len(rows) - len(template_ids) != 54:
        errors.append("expected 54 fixed task packages")
    ...
    print(
        "PASS: 57 registry entries; fixed=54; templates=3; "
