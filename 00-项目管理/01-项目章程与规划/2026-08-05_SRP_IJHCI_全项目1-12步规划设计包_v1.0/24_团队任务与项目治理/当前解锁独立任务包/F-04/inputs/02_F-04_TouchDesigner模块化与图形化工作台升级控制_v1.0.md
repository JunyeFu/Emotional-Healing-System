...
> 状态：`ACTIVE_CONTROL / F-04_IN_REVIEW / AC1-AC6_MACHINE_PASS / AC7_PENDING`
> 生效日期：2026-08-29
> 状态权威：`05_可领取任务包.csv`
...
- **AC7**：傅钧烨独立打开`.toe`、切换全部页面和场景、检查节点权限并签署`PASS/FAIL`；自动检查不得替代第二人结论。
...
5. 只有AC1至AC7全部关闭且傅钧烨签署`PASS`后，F-04才可转为`DONE`；本控制文件本身不构成实现完成或人工签收。
