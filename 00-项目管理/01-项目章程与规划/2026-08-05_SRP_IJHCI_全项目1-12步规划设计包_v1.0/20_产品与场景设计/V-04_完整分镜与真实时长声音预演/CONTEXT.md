...
| H3可拆层背景重建 | `PENDING` | 按已选方向建立四张可拆层背景；完成前V-04继续`IN_PROGRESS` |
...
旧H3合并机器门和独立复审结果作为历史证据保留。团队总监已选择`storm=B, heat=C, snow=C, fade=C`。下一硬门为`H3_LAYERABLE_BACKGROUND_REBUILD`；完成四张可拆层背景前，V-04继续保持`IN_PROGRESS`且不解锁V-05。
