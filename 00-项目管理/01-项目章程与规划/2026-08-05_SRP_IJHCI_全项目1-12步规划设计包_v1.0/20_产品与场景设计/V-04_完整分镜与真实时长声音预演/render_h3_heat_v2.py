...
def review_frame(scene: Image.Image, abstract: Image.Image, t_s: float, metrics: dict[str, object]) -> Image.Image:
    ...
    draw.text((36, 18), "V-04 H3 INPUT / HEAT / CANDIDATE-V2", font=title_font, fill=(244, 244, 240))
    draw.text(
        (36, 68),
        f"t={t_s:05.2f}s  target={target['phase']}  actual={actual['phase']}  shared fixed camera/haze/audio",
        font=label_font,
        fill=(201, 201, 194),
    )
...
def render_preflight(plate: Image.Image, config: dict[str, object]) -> None:
    output = REPO / ".artifacts-local/V-04/H3/heat-preflight-v2"
...
def main() -> None:
    ...
    require(config["schema_version"] == "2.0", "heat config schema drift")
    require(config["technical_id"] == "heat" and config["preview_id"] == "heat-candidate-v2", "heat identity drift")
    require(config["camera"] == {"mode": "fixed", "horizontal_scroll": False, "camera_displacement_px": 0.0, "source_fit": "cover_center_crop"}, "heat fixed camera drift")
    require(h2["decision"] == "PASS" and h2["results"]["H2"] == "PASS", "H2 has not passed")
    require(background_selection["selection"]["heat"]["candidate_id"] == "R2-C-heat", "heat background selection drift")
    ...
    require(source.is_file() and contract.is_file() and mechanism_contract.is_file(), "heat source or contract missing")
    ...
    with tempfile.TemporaryDirectory(prefix="heat-candidate-v2-build-", dir=output_root.parent) as temporary:
        ...
        manifest = {
            ...
            "next_if_pass": "team-director review with storm-candidate-v2 and snow-candidate-v2",
    ...
    print(
        "PASS: rendered heat-candidate-v2; 300 shared fixed-camera frames; "
