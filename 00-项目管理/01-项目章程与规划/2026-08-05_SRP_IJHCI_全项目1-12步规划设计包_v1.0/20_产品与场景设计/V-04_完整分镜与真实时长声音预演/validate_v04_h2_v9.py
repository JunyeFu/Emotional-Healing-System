...
CONTRACT_PATH = HERE / "V-04_H2_candidate-v9_色潮回流设计合同_v1.0.md"
...
def main() -> None:
    ...
    require(config.get("schema_version") == "1.2", "H2 v9 config schema drift")
    require(config.get("candidate_id") == "candidate-v9", "H2 v9 candidate drift")
    require(manifest.get("schema_version") == "1.2", "H2 v9 manifest schema drift")
    require(manifest.get("candidate_id") == config["candidate_id"], "H2 v9 identity mismatch")
    require(manifest.get("gate_status") == "PENDING_HUMAN_CONFIRMATION", "H2 v9 human gate drift")
    require(manifest.get("config_sha256") == sha256(CONFIG_PATH), "H2 v9 config hash drift")
    require(manifest.get("design_contract_sha256") == sha256(CONTRACT_PATH), "H2 v9 contract hash drift")
    ...
    validate_sheet(outputs["review_keyframes"], "fade-H2-keyframes-v9.jpg")
    validate_sheet(outputs["grayscale_keyframes"], "fade-H2-grayscale-v9.jpg")
    ...
    require(manifest["asset_status"]["formal_use_allowed"] is False, "H2 v9 cannot be formal-use enabled")
    require(bool(git_lines("check-ignore", "--", config["outputs"]["artifact_root"])), "H2 v9 media root must be ignored")
    require(not git_lines("ls-files", "--", config["outputs"]["artifact_root"]), "H2 v9 media must not be tracked")
    print(
        "PASS: V-04 H2 candidate-v9 machine gate; media, color envelope, water clipping, "
