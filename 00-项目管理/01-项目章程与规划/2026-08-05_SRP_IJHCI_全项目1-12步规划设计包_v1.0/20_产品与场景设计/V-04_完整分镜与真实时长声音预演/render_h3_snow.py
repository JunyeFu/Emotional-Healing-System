...
def prepare_scroll_plate(source: Image.Image, target_height: int) -> Image.Image:
    ...
    require(target_width >= 2304, "snow source plate is too narrow for the frozen scroll")
...
def review_frame(scene: Image.Image, abstract: Image.Image, t_s: float, metrics: dict[str, object]) -> Image.Image:
    ...
    draw.text((36, 18), "V-04 H3 INPUT / SNOW / CANDIDATE-V1", font=title_font, fill=(244, 246, 248))
    draw.text(
        (36, 68),
        f"t={t_s:05.2f}s  target={target['phase']}  actual={actual['phase']}  shared scroll/snow/fog/audio",
        font=label_font,
        fill=(202, 208, 215),
    )
...
def render_preflight(plate: Image.Image, config: dict[str, object]) -> None:
    output = REPO / ".artifacts-local/V-04/H3/snow-preflight-v1"
...
def main() -> None:
    ...
    require(config["schema_version"] == "1.0", "snow config schema drift")
    require(config["technical_id"] == "snow" and config["preview_id"] == "snow-candidate-v1", "snow identity drift")
    require(h2["decision"] == "PASS" and h2["results"]["H2"] == "PASS", "H2 has not passed")
    require(any(item["candidate_id"] == "snow-B" for item in h1["selections"]), "snow-B is not H1 selected")
    source = REPO / config["source"]["panorama_file"]
    contract = HERE / config["design_contract"]
    require(source.is_file() and contract.is_file(), "snow source or design contract missing")
    ...
    with tempfile.TemporaryDirectory(prefix="snow-candidate-v1-build-", dir=output_root.parent) as temporary:
        ...
        manifest = {
            ...
            "next_if_pass": "continue corridor input preview before H3 assembly",
    ...
    print(
        "PASS: rendered snow-candidate-v1; 300 shared frames; "
