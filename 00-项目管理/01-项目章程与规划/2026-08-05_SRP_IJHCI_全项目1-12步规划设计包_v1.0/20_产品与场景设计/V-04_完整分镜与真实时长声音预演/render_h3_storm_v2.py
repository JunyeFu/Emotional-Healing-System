...
CONFIG_PATH = HERE / "V-04_H3_storm样片配置_v2.0.json"
...
MANIFEST_PATH = HERE / "V-04_H3_storm候选清单_v2.0.json"
SIZE = (1920, 1080)
...
def review_frame(scene: Image.Image, abstract: Image.Image, t_s: float, metrics: dict[str, object]) -> Image.Image:
    ...
    draw.text((36, 18), "V-04 H3 INPUT / STORM / CANDIDATE-V2", font=title_font, fill=(244, 244, 244))
    draw.text(
        (36, 68),
        f"t={t_s:05.2f}s  target={target['phase']}  actual={actual['phase']}  shared fixed camera/weather/audio",
        font=label_font,
        fill=(195, 202, 208),
    )
...
def main() -> None:
    ...
    require(config["schema_version"] == "2.0", "storm config schema drift")
    require(config["technical_id"] == "storm" and config["preview_id"] == "storm-candidate-v2", "storm identity drift")
    require(config["camera"] == {"mode": "fixed", "horizontal_scroll": False, "camera_displacement_px": 0.0, "source_fit": "cover_center_crop"}, "storm fixed camera drift")
    require(h2["decision"] == "PASS" and h2["results"]["H2"] == "PASS", "H2 has not passed")
    require(background_selection["selection"]["storm"]["candidate_id"] == "R2-B-storm", "storm background selection drift")
    ...
    require(source.is_file() and contract.is_file() and mechanism_contract.is_file(), "storm source or contract missing")
    ...
    with tempfile.TemporaryDirectory(prefix="storm-candidate-v2-build-", dir=output_root.parent) as temporary:
        ...
        manifest = {
            ...
            "next_if_pass": "team-director review with heat-candidate-v2 and snow-candidate-v2",
    ...
    print(
        "PASS: rendered storm-candidate-v2; 360 shared fixed-camera frames; "
