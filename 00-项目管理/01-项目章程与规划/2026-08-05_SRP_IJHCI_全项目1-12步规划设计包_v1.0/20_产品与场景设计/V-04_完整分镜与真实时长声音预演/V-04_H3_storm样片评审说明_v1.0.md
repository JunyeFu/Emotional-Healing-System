# V-04 H3 storm样片评审说明 v1.0

> 状态：`MACHINE_PASS_READY_FOR_H3_ASSEMBLY`

## 评审对象

`storm-candidate-v1`是H3输入短样片，不单独设置人工硬门。它用于检查雨幕风门能否在12秒内清晰呈现`INHALE → HOLD_1 → EXHALE → HOLD_2`，并为最终H3合并评审提供storm片段。
...
1. 吸气时雨幕边界向外展开且拱顶抬升；第一次停留时边界稳定。
2. 呼气时风门回到中性宽度但不表现为环境恶化；第二次停留保持稳定。
3. 近场气流实际痕迹比目标滞后，且不被自动吸附。
4. 抽象条件中雨幕风门保持中性，外环和内环分别表达目标与实际。
5. 两条件的背景卷动、基础雨势、声音和时轴一致；基础天气不泄露3秒阶段边界。

storm机器门通过后继续制作heat、snow和山雾长廊输入样片。四项输入齐备后统一进入H3合并人工评审。
