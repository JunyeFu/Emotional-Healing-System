...
def review_frame(scene: Image.Image, abstract: Image.Image, t_s: float) -> Image.Image:
    ...
    draw.text((36, 18), "V-04 H2 / FADE / CANDIDATE-V11", font=title_font, fill=(244, 244, 244))
...
def make_keyframe_sheet(
    ...
    *,
    ...
    title = "V-04 H2 / FADE V11 / GRAYSCALE GEOMETRY" if grayscale else "V-04 H2 / FADE V11 / KEYFRAME REVIEW"
...
def main() -> None:
    ...
    require(config["schema_version"] == "1.4", "config schema drift")
    require(config["gate_id"] == "H2" and config["candidate_id"] == "candidate-v11", "config identity drift")
    ...
    require(design_contract.is_file(), "candidate-v11 design contract missing")
