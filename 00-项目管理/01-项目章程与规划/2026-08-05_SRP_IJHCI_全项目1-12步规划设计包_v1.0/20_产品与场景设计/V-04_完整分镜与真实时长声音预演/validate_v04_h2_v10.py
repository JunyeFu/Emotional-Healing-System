...
CONTRACT_PATH = HERE / "V-04_H2_candidate-v10_固定镜头与背景重建设计合同_v1.0.md"
...
def main() -> None:
    ...
    require(config.get("schema_version") == "1.3", "H2 v10 config schema drift")
    require(config.get("candidate_id") == "candidate-v10", "H2 v10 candidate drift")
    require(manifest.get("schema_version") == "1.3", "H2 v10 manifest schema drift")
    require(manifest.get("candidate_id") == config["candidate_id"], "H2 v10 identity mismatch")
    require(manifest.get("gate_status") == "PENDING_HUMAN_CONFIRMATION", "H2 v10 human gate drift")
    require(manifest.get("config_sha256") == sha256(CONFIG_PATH), "H2 v10 config hash drift")
    require(manifest.get("design_contract_sha256") == sha256(CONTRACT_PATH), "H2 v10 contract hash drift")
    ...
    require(root.is_dir(), "H2 v9 candidate root missing")
    ...
    validate_sheet(outputs["review_keyframes"], "fade-H2-keyframes-v10.jpg")
    validate_sheet(outputs["grayscale_keyframes"], "fade-H2-grayscale-v10.jpg")
    ...
    require(manifest["asset_status"]["formal_use_allowed"] is False, "H2 v10 cannot be formal-use enabled")
    require(bool(git_lines("check-ignore", "--", config["outputs"]["artifact_root"])), "H2 v10 media root must be ignored")
    require(not git_lines("ls-files", "--", config["outputs"]["artifact_root"]), "H2 v10 media must not be tracked")
    print(
        "PASS: V-04 H2 candidate-v10 machine gate; fixed camera, media, color envelope, water clipping, "
