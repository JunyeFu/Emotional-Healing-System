...
G-02只对外返回不透明预约和审计ID。去重库、研究编号映射库和Windows凭据必须分离；正式专机、保留期限授权或Unity许可门任一未通过时失败关闭。实现与证据入口见`02-技术研发/07-数据治理/README.md`。
...
- Unity现有资产和场景只作迁移参考；V-01、V-02与V-03已由真实团队第二人签收并转为`DONE`，不代表Unity运行完成；V-02签收对象为提交`3ebbdee`，签收提交为`c04a7be`；V-03最终候选`f09f0d5`经独立Agent复审及傅钧烨真实团队第二人复核均为`PASS`，签收提交为`e6d642e`；其7类78项资产台账仍未获得正式许可放行，`fade`仅冻结为U-03设计交接对象；F-03实现候选`a61eba6`经独立Agent复审和傅钧烨第二人确认后转为`DONE`，只提供可复现Unity环境、测试与Windows `DEV-REPLAY`构建底座；F-05由Codex Agent在`codex/f-05-phase-v22`认领并进入`IN_PROGRESS`，独立承担`storm`双停与`fade`双吸的相位实例v2.2、P-01/P-02适配及Unity/TD消费合同fixture；V-04保持`IN_PROGRESS`并等待团队总监H3确认；U-01、U-02、T-01、U-07和V-05仍有未关闭依赖，目标四层接口、两条件、实时通信和完整会话旅程仍未实现；
...
- P-01会话核心与P-02追加存储已由真实团队第二人签收并转为`DONE`；该状态只关闭各自任务门，正式模式仍因X-01、G-02正式门、Unity客户端和真实输入链未闭合而失败关闭。
