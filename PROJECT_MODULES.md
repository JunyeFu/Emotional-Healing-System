...
- Unity现有资产和场景只作迁移参考；V-01研究体验契约与完整旅程已由真实团队第二人签收并转为`DONE`，不代表Unity运行完成；V-02候选提交`3ebbdee`已通过模型独立复审并转为`IN_REVIEW`，真实团队第二人签收前V-03继续`WAIT_DEP`；F-05独立承担`storm`双停与`fade`双吸的相位实例v2.2及消费者迁移，V-03至V-05后续负责四层映射合同与工程风险排序、分镜声音预演和全旅程灰盒，目标四层接口、双条件和完整会话状态机仍未实现；
...
- P-01已形成Python会话核心与本机传输技术候选；P-02已形成追加存储与重放技术候选，但二者尚未完成真实团队第二人签收和正式运行装配，正式模式仍因X-01、G-02正式门和Unity客户端未闭合而失败关闭。
- P-02已形成L0/L1追加、哈希链、封存、数据恢复和确定性重放技术候选；第二人复核及正式环境联调未完成前不得据此开放正式运行。
