...
| ID | 模块 | 输入 | 输出 | 主要实现 |
|---|---|---|---|---|
| M02 | 数据采集 | 真实设备或开发适配器 | 原始样本、设备状态、质量元数据 | `01-数据采集/` |
| M03 | 交互状态估计 | 同步设备帧 | 目标/实际事件、质量、PF候选和累计状态 | `02-信号处理/` |
| M04 | 运行合同与记录 | Python权威状态 | v2.1合同、可靠控制、20Hz遥测、L0/L1 | `05-通信协议/` |
| M05 | TD只读操作台 | UDP `5005` | 监控、人工标记和中止请求 | `03-TouchDesigner/` |
| M06 | Unity参与者制品 | TCP `5010`、UDP `5006` | 四天气完整体验、ACK和渲染回执 | `04-Unity视觉/` |
